...
// TPUAsrModel flags
DEFINE_string(tpu_model_dir, "",
  "directory where the SOPHGO TPU model is saved");
...
namespace wenet {
  ...
  std::shared_ptr<DecodeResource> InitDecodeResourceFromFlags() {
    ...
    else if (!FLAGS_tpu_model_dir.empty())
    {
#ifdef USE_TPU
      LOG(INFO) << "Reading SOPHGO TPU model from " << FLAGS_tpu_model_dir;
      auto model = std::make_shared<TPUAsrModel>();
      model->Read(FLAGS_tpu_model_dir);
      resource->model = model;
#else
      LOG(FATAL) << "Please rebuild with cmake options '-DTPU=ON'.";
#endif
    }
    else {
      LOG(FATAL) << "Please set ONNX, TORCH, XPU, BPU, TPU or OpenVINO model path!!!";
    }
